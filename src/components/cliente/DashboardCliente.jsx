...
const DashboardCliente = () => {
  ...
  return (
    <div className={`min-h-screen flex ${darkMode ? "bg-[#0B0F18]" : "bg-gray-100"}`}>
      <div className="flex-shrink-0">
        <SidebarCliente />
      </div>
      <div
        className={`flex-1 pt-6 px-6 pb-2 w-full ${darkMode ? "text-white" : "text-gray-900"} transition-colors`}
      >
        <h1 className="text-3xl font-bold mb-2">{t("dashboard")}</h1>
        <p className="mb-6 text-base text-blue-300">{t("bienvenido_de_nuevo")}</p>
        <p className="mb-6 text-4xl text-blue-500">{t("ultimas_estadisticas_del_mes")}</p>

        <div className="mb-8">
          <div className="flex gap-4 overflow-x-auto md:grid md:grid-cols-4 md:gap-4">
            <div className={`min-w-[220px] md:min-w-0 rounded-xl p-4 md:p-5 shadow-lg border ${darkMode ? "bg-[#232846] border-blue-700" : "bg-white border-blue-300"}`}>
        ...
        <div className="mb-4">
              ...
              {currentItems.map((op, idx) => (
                ...
                >
                  ...
                  {expandedOperationId === op.id_operacion && (
                    <div className={`pt-3 border-t w-full text-sm 
          ...
          {ultimasOperaciones.length > itemsPerPage && (
            <div className={`flex justify-center items-center mt-2 space-x-4 ${darkMode ? 'text-white' : 'text-gray-700'}`}>
              <button
                onClick={prevPage}
