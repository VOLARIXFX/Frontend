...
import DashboardSidebar from "./cliente/SidebarCliente"
...
const EstadisticasPage = () => {
  // NUEVOS ESTADOS DE FILTRO (basados en TradingFilters)
  ...
  // Hook para estadísticas anuales en tiempo real (para las gráficas)
  useEffect(() => {
  ...
  // Generar años disponibles (2020 hasta actual)
  const generateYears = () => {
    const currentYear = new Date().getFullYear()
    const years = []
  ...
  // Calcular número de semanas en un mes específico
  ...
  // Obtener años con datos (simulado por ahora)
  ...
  // Funciones de formato de moneda y porcentaje
        ...
        <div className="p-2 sm:p-4 lg:p-6">
          <div className="max-w-7xl mx-auto space-y-4 lg:space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
                  ...
                  >
                    <div className="text-center">
                      ...
                      <div className="font-medium">{t('periodo_actual')}</div>
                      <div className="text-sm opacity-75">{t('datos_actuales')}</div>
                    </div>
                  </button>
                  <button
                  ...
                  >
                    <div className="text-center">
                      <div className="text-2xl mb-2">📅</div>
                      <div className="font-medium">{t('semanas_pasadas')}</div>
                      <div className="text-sm opacity-75">{t('por_mes_y_semana')}</div>
                    </div>
                  </button>
                  <button
                  ...
                  >
                    <div className="text-center">
                      ...
                      <div className="font-medium">{t('meses_pasados')}</div>
                      <div className="text-sm opacity-75">{t('cualquier_mes')}</div>
                    </div>
                  </button>
                  <button
                  ...
                  >
                    <div className="text-center">
                      <div className="text-2xl mb-2">📆</div>
                      <div className="font-medium">{t('semestres_pasados')}</div>
                      <div className="text-sm opacity-75">{t('por_semestre')}</div>
                    </div>
                  </button>
                  <button
                  ...
                  >
                    <div className="text-center">
                      <div className="text-2xl mb-2">🗃️</div>
                      <div className="font-medium">{t('años_pasados')}</div>
                      <div className="text-sm opacity-75">{t('cualquier_año')}</div>
                    </div>
                  </button>
                </div>
              </div>
              <div className={`${darkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-sm p-4 mb-4`}>
                <h2 className={`text-base font-semibold ${darkMode ? "text-white" : "text-gray-900"} mb-3`}>{t('configuracion_filtro')}</h2>
                {/* Período Actual */}
                {filterType === "actual" && (
                  <div className="space-y-4">
                    <div>
                      <label className={`block text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"} mb-2`}>
                        {t('selecciona_periodo')}
                      </label>
                      <select
                        value={actualPeriod}
                      ...
                      >
                        <option value="semanal">{t('semana_actual')}</option>
                        <option value="mensual">{t('mes_actual')}</option>
                        <option value="semestral">{t('semestre_actual')}</option>
                        <option value="anual">{t('año_actual')}</option>
                      </select>
                    </div>
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <p className="text-blue-800">
                        <strong>{t('periodo_seleccionado')}:</strong> {getCurrentPeriodLabel()}
                      </p>
                      <p className="text-blue-600 text-sm mt-1">
                        {t('filtro_periodo_actual')}
                      </p>
                    </div>
                  </div>
                )}
                ...
                {filterType === "semanas_pasadas" && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <label className={`block text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"} mb-2`}>{t('año')}</label>
                        <select
                          value={selectedYear}
                      ...
                      <div>
                        <label className={`block text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"} mb-2`}>{t('mes')}</label>
                        <select
                          value={selectedMonth}
                          onChange={(e) => {
                      ...
                      <div>
                        <label className={`block text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"} mb-2`}>{t('semana')}</label>
                        <select
                          value={selectedWeek}
                        ...
                        >
                          {Array.from({ length: getWeeksInMonth(selectedYear, selectedMonth) }, (_, i) => (
                            <option key={i + 1} value={i + 1}>{t('semana')} {i + 1}</option>
                          ))}
                        </select>
                      </div>
                    </div>
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                      ...
                      <p className="text-green-600 text-sm mt-1">{t('semana_especifica')}</p>
                    </div>
                  </div>
                )}
                {/* Meses Pasados */}
                {filterType === "meses_pasados" && (
                  <div className="space-y-4">
                    ...
                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                      ...
                      <p className="text-purple-600 text-sm mt-1">{t('mes_completo')}</p>
                    </div>
                  </div>
                )}
                {/* Semestres Pasados */}
                {filterType === "semestres_pasados" && (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className={`block text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"} mb-2`}>{t('semestre')}</label>
                        <select
                    ...
                    <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
                      ...
                      <p className="text-orange-600 text-sm mt-1">{t('semestre_completo')}</p>
                    </div>
                  </div>
                )}
                {/* Años Pasados */}
                {filterType === "años_pasados" && (
                  <div className="space-y-4">
                    <div>
                      <label className={`block text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"} mb-2`}>{t('selecciona_año')}</label>
                      <select
                    ...
                    <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                      ...
                      <p className="text-red-600 text-sm mt-1">{t('año_completo')}</p>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className={`${darkMode ? "bg-gray-800/90" : "bg-white/90"} backdrop-blur-sm rounded-xl lg:rounded-2xl shadow-xl border ${darkMode ? "border-gray-700" : "border-gray-200"} p-3 sm:p-4 lg:p-6`}>
              <div className="mb-2">
                <h3 className={`text-base sm:text-lg font-bold ${darkMode ? "text-white" : "text-gray-900"}`}>{t('resumen_visual')}</h3>
                <p className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-600"}`}>{t('explora_rendimiento')}</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Gráfica de barras: Ganadoras vs Perdedoras */}
                <div className="flex flex-col items-center justify-center">
                  <h4 className={`mb-2 font-semibold ${darkMode ? "text-white" : "text-gray-800"}`}>{t('operaciones')}</h4>
                  <ReactApexChart
                    options={{
                ...
                <div className="flex flex-col items-center justify-center">
                  <h4 className={`mb-2 font-semibold ${darkMode ? "text-white" : "text-gray-800"}`}>{t('promedio')}</h4>
                  <ReactApexChart
                    options={{
                      chart: { type: 'bar', toolbar: { show: false } },
                ...
                <div className="flex flex-col items-center justify-center">
                  <h4 className={`mb-2 font-semibold ${darkMode ? "text-white" : "text-gray-800"}`}>{t('distribucion')}</h4>
                  {(ganadoras + perdedoras) > 0 ? (
                    ...
                    <span className="text-gray-400 text-sm mt-4">{t('no_datos_distribucion')}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
              ...
              >
                ...
                <div className="mt-3 sm:mt-4">
                  ...
                  >
                    {rentabilidad >= 0 ? t('positiva') : t('negativa')}
                  </span>
                </div>
              </div>
